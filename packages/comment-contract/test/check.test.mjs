...
describe("changedFiles", () => {
	...
	test("reads every file's added ranges from one diff", () => {
		...
		const spans = addedRangesByFile(from, [...changedFiles(from), "apps/main/src/a.ts"]);

		assert.deepEqual(spans.get("apps/main/src/b.tsx"), [{ from: 1, to: 1 }]);
		assert.deepEqual(spans.get("scripts/ci/tool.mjs"), [{ from: 1, to: 1 }]);
		assert.deepEqual(spans.get("apps/main/src/renamed.ts"), [{ from: 1, to: 1 }]);
	});
	...
	test("holds nothing for a path outside the set it was given", () => {
		...
		assert.equal(
			addedRangesByFile(from, ["apps/main/src/b.tsx"]).has("scripts/ci/tool.mjs"),
			false,
		);
	});
});
...
describe("renamedFrom", () => {
	...
	test("maps a renamed path back to its old name", () => {
		assert.equal(renamedFrom(mergeBase("base-marker")).get("src/new.ts"), "src/old.ts");
	});

	test("holds nothing for a file that did not move", () => {
		assert.equal(renamedFrom(mergeBase("base-marker")).has("src/kept.ts"), false);
	});
