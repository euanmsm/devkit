...
export function renamedFrom(from) {
	...
	for (const line of run(["diff", "--name-status", RENAMES, from, "HEAD"]).split("\n")) {
...
export function addedRangesByFile(from, paths) {
	...
	for (const line of run(["diff", "-U0", RENAMES, from, "HEAD", "--", ...paths]).split("\n")) {
...
function checkFile(file, from, renames, spans) {
	...
	const span =
		spans.get(file) ?? addedRanges(run(["diff", "-U0", RENAMES, from, "HEAD", "--", was, file]));
...
export function main() {
	...
	const spans = addedRangesByFile(from, [...files, ...files.map((f) => renames.get(f) ?? f)]);
	let total = 0;
	...
	if (total === 0) {
		console.log(`No new comment-contract violations across ${files.length} changed file(s).`);
		process.exit(0);
	}

	console.log(`\n${total} new violation(s) across ${files.length} changed file(s).`);
	if (config.rulesDoc) console.log(`The rules are in ${config.rulesDoc}`);
	if (config.examplesDoc) console.log(`Paired examples are in ${config.examplesDoc}`);
	process.exit(1);
}
