...
# Called from the pre-commit hook alongside config-integrity.sh.
#
# Config comes from the wrapper in bin/security-scan.mjs, which reads
# .devkit/repo-security.json and passes it in as DEVKIT_* variables.
#
# Tools
# -----
...
# Usage
# -----
#   devkit-security-scan               # CI mode (scans the whole tree)
#   PRE_COMMIT=1 devkit-security-scan   # pre-commit mode (staged files only)
