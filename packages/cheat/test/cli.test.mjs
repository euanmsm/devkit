...
// scratch CHEAT_STATE — never the real one in the home directory.
function scratchState() {
  return join(mkdtempSync(join(tmpdir(), 'cheat-test-')), 'progress.json');
}

async function learn(state, ...argv) {
  const c = capture({ CHEAT_STATE: state });
...
describe('no arguments', () => {
  ...
  it('points at the learning half', async () => {
    const { out } = await cheat();
    match(out, /cheat learn\s+Courses to work through/);
  });
});

describe('a tool on its own', () => {
  it('shows the menu, not the content', async () => {
    const { code, out } = await cheat('gh');
    equal(code, 0);
    match(out, /Topics:\s+cheat gh --<topic>/);
...
describe('one topic', () => {
  it('prints only that topic', async () => {
    const { code, out } = await cheat('gh', '--pr');
    equal(code, 0);
  ...
  it('treats the leading -- as optional', async () => {
    const withDashes = await cheat('gh', '--pr');
    const without = await cheat('gh', 'pr');
    equal(without.out, withDashes.out);
  });

  it('accepts a unique prefix', async () => {
    const short = await cheat('gh', '--bro');
    const full = await cheat('gh', '--browse');
    equal(short.out, full.out);
  });

  it('prints several when asked for several', async () => {
    const { out } = await cheat('gh', '--pr', '--issue');
    match(out, /gh pr create/);
...
describe('the whole guide', () => {
  it('includes every topic', async () => {
    const { code, out } = await cheat('gh', '--all');
    equal(code, 0);
...
describe('mistakes', () => {
  it('shows the menu and fails on an unknown topic', async () => {
    const { code, out } = await cheat('gh', '--nope');
    equal(code, 1);
  ...
  it('lists the candidates and fails on an ambiguous prefix', async () => {
    const { code, err } = await cheat('git', '--s');
    equal(code, 1);
  ...
  it('names the tools it does know on an unknown tool', async () => {
    const { code, err } = await cheat('nosuchtool');
    equal(code, 1);
...
describe('the learning half', () => {
  ...
  it("shows a course's lessons, not their content", async () => {
    ...
    equal(code, 0);
    match(out, /Lessons, in order:\s+cheat learn shell --<lesson>/);
    match(out, /\n\s+line-editor\s/);
  ...
  it('prints a lesson, with the next one signposted', async () => {
    ...
    match(out, /Ctrl-A/);
    match(out, /Next:\s+cheat learn shell --history/);
  });
...
describe('practice and drills need a terminal', () => {
  it('refuses to practise into a pipe, in one line', async () => {
    const c = capture({ CHEAT_STATE: scratchState() });
    const code = await run(['learn', 'shell', '--practice'], c.io);
    equal(code, 1);
    match(c.errText(), /^cheat: practice needs a terminal$/);
  ...
  it('refuses to drill into a pipe', async () => {
    ...
    equal(code, 1);
    match(c.errText(), /^cheat: drill needs a terminal$/);
  });
...
describe('interactive routing', () => {
  ...
  it('bare cheat opens the menus instead of printing', async () => {
...
describe('guides with no menu', () => {
  it('prints lsof in full, exactly as recorded', async () => {
    const { code, out } = await cheat('lsof');
    equal(code, 0);
